...
T readUnaligned(T, bool consume)(ref ubyte* p)
{
...
void _d_getLanguageSpecificTables(ubyte* data, ref ubyte* callsite, ref ubyte callsiteEncoding, ref ubyte* action, ref ubyte* classinfo_table, ref ubyte ciEncoding)
{
    if (data is null)
    {
...
extern(C) auto eh_personality_common(NativeContext)(ref NativeContext nativeContext)
{
    ...
    _d_getLanguageSpecificTables(data, callsite_table, callsite_encoding, action_table, classinfo_table, classinfo_table_encoding);
    ...
    else // !SjLj_Exceptions
    {
        ...
        while (true)
        {
            ...
            static ulong read_value(ubyte encoding, ref ubyte* p)
            {
                switch (encoding)
                {
                    case _DW_EH_Format.DW_EH_PE_sdata2: return readUnaligned!(short,  true)(p);
                    case _DW_EH_Format.DW_EH_PE_udata2: return readUnaligned!(ushort, true)(p);
                    case _DW_EH_Format.DW_EH_PE_sdata4: return readUnaligned!(int,    true)(p);
                    case _DW_EH_Format.DW_EH_PE_udata4: return readUnaligned!(uint,   true)(p);
                    case _DW_EH_Format.DW_EH_PE_sdata8: return readUnaligned!(long,   true)(p);
                    case _DW_EH_Format.DW_EH_PE_udata8: return readUnaligned!(ulong, true)(p);
                    case _DW_EH_Format.DW_EH_PE_uleb128:
                        size_t val;
                        p = get_uleb128(p, val);
                        return val;
                    case _DW_EH_Format.DW_EH_PE_sleb128:
                        ...
                        return val;
                    default:
                        fatalerror("Unsupported call site format");
                        assert(0);
                }
            }

            immutable block_start_offset = read_value(callsite_encoding, callsite_walker);
            immutable block_size = read_value(callsite_encoding, callsite_walker);
            landingPadAddr = cast(ptrdiff_t) read_value(callsite_encoding, callsite_walker);
